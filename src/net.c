...
int mqtt3_socket_accept(struct mosquitto_db *db, mosq_sock_t listensock)
{
	...
	if (( rcv_length = read(new_sock, &connType, sizeof(connType))) <0){
		perror("read");
		exit(1);
	}	
	printf("rcv_length : %d\n", rcv_length);
	if(rcv_length > 0)
		printf("Msg from connecting node : %s\n", connType);

	if(!strcmp(connType, "client")){
		sprintf(slave, "163.180.117.97:10000");	
		slave[21] = '\n';
		printf("Send slave ip addr : %s\n", slave);
		if(write(new_sock, &slave, sizeof(slave)) < 0){
			perror("write");
			exit(1);
		}
