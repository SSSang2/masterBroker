	...
	if(db->config->connection_messages == true){
		if(context->is_bridge){
			...
			if(context->username){
				...
				_mosquitto_log_printf(NULL, MOSQ_LOG_NOTICE, "New client connected from %s as %s (c%d, k%d, p%d).", context->address, client_id, clean_session, context->keepalive, context->listener->port);
			}
		}
	}
